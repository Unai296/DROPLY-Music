...
-- ────────────────────────────────────────────────────────────
-- 9. CACHED_URLS — audio URLs cacheadas para background playback
--     Se usa cuando la pantalla está bloqueada en iOS.
--     Las URLs expiran (~6h), así que se refrescan periódicamente.
-- ────────────────────────────────────────────────────────────
create table if not exists public.cached_urls (
...
alter table public.cached_urls enable row level security;

drop policy if exists "cached_urls_select_anon" on public.cached_urls;
drop policy if exists "cached_urls_insert_anon" on public.cached_urls;
drop policy if exists "cached_urls_update_anon" on public.cached_urls;
drop policy if exists "cached_urls_delete_anon" on public.cached_urls;

-- Cualquiera (incluso sin auth) puede leer y escribir la caché de URLs
create policy "cached_urls_select_anon" on public.cached_urls
  for select using (true);

create policy "cached_urls_insert_anon" on public.cached_urls
  for insert with check (true);

create policy "cached_urls_update_anon" on public.cached_urls
  for update using (true) with check (true);

create policy "cached_urls_delete_anon" on public.cached_urls
  for delete using (true);

-- ────────────────────────────────────────────────────────────
-- 10. EDGE FUNCTION CONFIG (opcional, para mantener API Key)
--     Guarda la YouTube Data API Key de forma segura.
-- ────────────────────────────────────────────────────────────
-- (Opcional: crear secreto en Supabase CLI)
-- supabase secrets set YOUTUBE_API_KEY="AIzaSyCzC3R5cZKXgFw6r4EumwIgM05bA3kKppY"
